...
class ReportGenerator:
    ...
    def generate(self, findings: Dict, format: str = "html",
                 output: str = "report.html", diagrams: Optional[List[str]] = None,
                 template: Optional[str] = None) -> str:
        if format == "html":
            return self._gen_html(findings, output, diagrams=diagrams, template=template)
        elif format == "pdf":
            return self._gen_pdf(findings, output, diagrams=diagrams)
        elif format == "markdown":
            return self._gen_markdown(findings, output, diagrams=diagrams)
        elif format == "json":
            return self._gen_json(findings, output)
        elif format == "sarif":
            return self._gen_sarif(findings, output)
        return ""

    # ------------------------------------------------------------------
    # SARIF export — OASIS static analysis results format
    # ------------------------------------------------------------------

    def _gen_sarif(self, findings: Dict, output: str) -> str:
        """Write a SARIF 2.1.0 log (CodeQL / GHAS compatible)."""
        from defihunter.core.sarif import export_sarif
        vulns = findings.get("vulnerabilities", [])
        export_sarif(
            vulns,
            output,
            tool_name="defi-hunter",
            tool_version=findings.get("tool_version", "unknown"),
            target=str(findings.get("target", "target")),
        )
        return output
    ...
    def _gen_html(self, findings: Dict, output: str,
                  diagrams: Optional[List[str]] = None,
                  template: Optional[str] = None) -> str:
        # Custom template support: a Jinja2 file with {{ findings_html }},
        # {{ counts }}, {{ risk_score }} etc.
        if template and Path(template).exists():
            try:
                return self._gen_html_custom_template(findings, output, template)
            except ImportError:
                pass  # jinja2 missing — fall back to builtin
        ...
        # Mermaid diagrams (call graph / attack flow / storage layout)
        diagrams_html = ""
        if diagrams:
            from defihunter.core.diagrams import render_diagram_markdown
            rendered = []
            for kind in diagrams:
                mmd = render_diagram_markdown(
                    kind,
                    target=target,
                    contracts=contracts,
                    findings=vulns,
                    sources=[c.get("source", "") for c in contracts.values()
                             if isinstance(c, dict) and c.get("source")],
                )
                if mmd:
                    body = mmd.replace("```mermaid\n", "").replace("\n```", "")
                    rendered.append(f"<div class='diagram'><pre class='mermaid'>{body}</pre></div>")
            if rendered:
                diagrams_html = (
                    "<h2>Diagrams</h2>" + "".join(rendered) +
...
<footer>
    ...
    def _gen_html_custom_template(self, findings: Dict, output: str, template: str) -> str:
        """Render a user-supplied Jinja2 HTML template.

        Template context:
          {{ target }}, {{ chain }}, {{ risk_score }}, {{ risk_label }},
          {{ counts }}, {{ contracts }}, {{ vulnerabilities }},
          {{ scan_time }}, {{ tool_version }}, {{ findings_html }}
        """
...
<div class="finding sev-{sev}">
    ...
    def _gen_pdf(self, findings: Dict, output: str,
                 diagrams: Optional[List[str]] = None) -> str:
        ...
        with tempfile.NamedTemporaryFile(suffix=".html", delete=False) as tmp:
            tmp_path = tmp.name
        try:
            self._gen_html(findings, tmp_path, diagrams=diagrams)
    ...
    def _gen_markdown(self, findings: Dict, output: str,
                      diagrams: Optional[List[str]] = None) -> str:
        target = findings.get("target", "Unknown")
        vulns = findings.get("vulnerabilities", [])
        chain = findings.get("chain", "ethereum")
        ...
        if diagrams:
            from defihunter.core.diagrams import render_diagram_markdown
            lines.append(f"## Diagrams")
            lines.append(f"")
            for kind in diagrams:
                mmd = render_diagram_markdown(
                    kind, target=target, contracts=findings.get("contracts", {}),
                    findings=vulns)
                if mmd:
                    lines.append(mmd)
                    lines.append(f"")
